...
def main() -> None:
    ...
    results_and = engine.search(query_and, top_k=3, mode="and", ranker="tfidf")
    results_or = engine.search(query_or, top_k=3, mode="or", ranker="tfidf")
    results_bm25 = engine.search(query_or, top_k=3, mode="or", ranker="bm25")
    ...
    print(f"\nOR Query with BM25: {query_or}")
    for rank, result in enumerate(results_bm25, start=1):
        print(f"{rank}. {result.doc_id} (score={result.score:.4f})")
